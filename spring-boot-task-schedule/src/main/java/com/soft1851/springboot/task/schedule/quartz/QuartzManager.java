...
/**
 * @Author: zw_w
 * @Date: 2020/5/18 17:58
 * @Description: 封装了Quartz动态添加、修改和删除定时任务时间的方法
 */
@Slf4j
...
//        //创建触发器对象
//        SimpleTrigger trigger = TriggerBuilder.newTrigger().withIdentity(triggerName,triggerGroupName).startAt(startTime).withSchedule(SimpleScheduleBuilder.simpleSchedule().withIntervalInSeconds(interval).repeatForever()).build();
//        //任务调度器
...
//    }
    ...
    public static void modifyJobTime(String triggerName,String triggerGroupName,int interval) throws SchedulerException{
        ...
        if (interval != oldInterval){
            Date startTime = new Date();
            startTime.setTime(startTime.getTime()+interval*1000);
            SimpleTrigger newTrigger = TriggerBuilder.newTrigger().withIdentity(triggerName,triggerGroupName).startAt(startTime).withSchedule(SimpleScheduleBuilder.simpleSchedule().withIntervalInSeconds(interval).repeatForever()).build();
            //修改任务的触发时间
    ...
    /**
     * 功能：删除一个任务，使用这个方法及即使任务都删除完了，程序也不会停止，因为调度器还在一直等待着任务
     * @param jobName           任务名
     * @param jobGroupName      任务组名
     * @param triggerName       触发器名
     * @param triggerGroupName  触发器组名
